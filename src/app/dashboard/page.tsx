              ...
              <div key={league.id} className="bg-white p-6 rounded-lg shadow">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-semibold !text-black flex items-center gap-2">
                    {league.game_type === 'fifa' ? <Gamepad2 className="h-6 w-6 text-gray-900" strokeWidth={2} /> : '🏓'} {league.name}
                  </h2>
                  {league.isMember && (
                ...
                {league.isMember ? (
                  <Link href={`/leaderboard/${league.id}`} className="w-full">
                    <Button className="w-full border-blue-600 !text-blue-600 hover:border-blue-700 hover:!text-blue-700" variant="outline">
                      View Leaderboard
                    </Button>
                  </Link>
                ) : (
